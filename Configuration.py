...
class Config:
    ...
    def get_max_value(self):
        try:
            with open(os.path.join(os.getcwd(), 'data', 'max_value.txt'), "r") as f:
                line = f.readline()
            self.max_value = np.array(list(map(float, (line.split()))))
        except:
    ...
    def computing_max_value(self):
        _max_value = [0.0 for i in range(self.input_l)]
        ...
        for i in tqdm(data.keys()):
            ...
            max_value_ = list(np.amax(x_data, axis=0))
            for i in range(len((max_value_))):
                if max_value_[i] > _max_value[i]:
                    _max_value[i] = max_value_[i]

        with open(os.path.join(os.getcwd(), 'data', 'max_value.txt'), 'w') as f:
            f.write(' '.join(map(str, _max_value)))
    ...
    def computing_mean_std(self):
        ...
        for i in tqdm(data.keys()):
            ...
            means = x_data.mean(dtype='float32', axis=0)
            stds = x_data.std(dtype='float32', axis=0)
